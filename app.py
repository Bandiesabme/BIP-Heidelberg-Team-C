...
sign_check_every = 4          # run every Nth frame
sign_trigger_frac = 0.35      # sign box height >= this fraction of frame -> ACTION
last_sign_frac = 0.0          # most recent detected sign size (telemetry)
last_sign_seen = None         # None, 'stop', 'left', 'right'

# Initialize detectors (Notice: no more model_path needed for the lightweight CV version)
    ...
    def do_GET(self):
        ...
        # --- API ROUTING ---
            ...
            if 'blue_h_min' in query:
                sign_detector.blue_h_min = int(query['blue_h_min'][0])
            if 'blue_h_max' in query:
                sign_detector.blue_h_max = int(query['blue_h_max'][0])
            if 'sign_s_min' in query:
                sign_detector.sign_s_min = int(query['sign_s_min'][0])
            if 'sign_v_min' in query:
                sign_detector.sign_v_min = int(query['sign_v_min'][0])
